  ...
  def successorStates(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not hitsWall:
        ...
        #visitedCorners.append(corners)
        nextState = state[1].copy()
        nextState[nextx][nexty] = False
        succ.append( ( ((nextx, nexty), nextState), action, 1) )
      "*** Your Code Here ***"
...
def cornersHeuristic(state, problem):
  ...
  while unvisit:
      for corner in unvisit:
          ...
          heurVal.append(distance)
  """
  for state in corners:
     ManDist = util.manhattanDistance(node, state)
     vals.append(ManDist)
